...
public:
    Worker() { 
        _th = new std::thread(&Worker::worker_main, this); }    // asio::thread는 인자 넘길수가 없음.
...
public:
    void Post(Task* task) { int index = task->_id % _count;
...
void task_test() {
    ...
    auto task1 = new Task(1, [a, &scheduler]() {    // 지역변수는 레퍼런스 캡쳐하면 안되지만 테스트 코드니 일단 쓴다.
        ...
        scheduler->Post(task2);
    });

    scheduler->Post(task1);
}

void network_test() {
    auto network = ari::Network::TCP(ari::IP::V4, 8080);
    network->Start();
}
